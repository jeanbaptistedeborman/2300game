...
const familyPresentationPageStyle = `
@page {
  size: A4 portrait;
  margin: 5mm;
}
...
export const generateFamilyPresentation = (cards: Card[], completedCards: Card[]) => {
  ...
  <style>
    ${styles}
    ${familyPresentationPageStyle}
    </style>
  
    <body style="zoom:1.25;font-size: larger">
       <header style="break-inside: avoid;margin-bottom: 2mm;"> <h1 style="margin:0 0 3mm 0;">Les tribus et leurs pouvoirs de base</h1>
        
        </header>
        
       <div style="column-count: 1; gap: 1mm 5mm;">     
       ${Object.keys(cardsByFamiy).map((key ) => {
            ...
            return `
           <div style="display: flex;margin: 0 0 1mm 0;background-color: ${family.color}; border:1mm solid; border-radius: 2mm; overflow: clip;border-color:${family.color};break-inside: avoid-column;">
           
        <div style="flex-grow: 1; zoom:1.5; display:flex; flex-direction:column;">
        ${primaryAbility ? `
                <div style="border:.5mm solid ${family.color}; background-color:${family.color}; border-radius:1mm; overflow:hidden;font-weight: 600;">
                  <div style="display:flex; align-items:stretch;">
                    <div style="flex-grow:1; min-width:0;">
                      <div style="padding: .2mm 1mm 0 1mm; font-weight: 700; font-size: 10pt; text-transform: uppercase; color: ${family.isDarkColor ? 'white' : 'black'}; display:flex; align-items:center; gap:1mm;">
                      ...
                      <div style="padding: .2mm 1mm .5mm 1mm; font-size: 9pt; line-height: 1.2em; background-color: ${family.color};">
                        <div style="background-color:white; color:black; border-radius:.6mm; width:100%; box-sizing:border-box; overflow:hidden;">
                          <div style="padding:.2mm .6mm; overflow:hidden;">
                            ...
                            ${primaryAbility.effect ? `<br/>${getEmplacementIcon('right')}
                EMPLACEMENTS VOISINS
                ${getEmplacementIcon('left')} <br/>` : ''}${primaryAbility.text || ''}
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                </div>` : `<div style="padding: .2mm 1mm .2mm 1mm; font-weight: 700; font-size: 10pt; text-transform: uppercase; color: ${family.isDarkColor ? 'white' : 'black'}; display:flex; align-items:center; gap:1mm;">
          <span style="display:inline-flex; width:2em; height:2em; align-items:center; justify-content:center; flex-shrink:0;">${family.icon}</span>
          <span>${family.familyName}</span>
