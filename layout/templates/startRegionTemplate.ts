...
export const getStartRegionTemplate = (index?: number): string => {
    ...
    return `<div class="card start-region ${isExperienced? 'experienced' : 'novice'}" >
            <img alt='start region' style="width:80%;height:80%" src="${url}">
             ${isExperienced? '<div style="position:absolute;bottom:0;width:100%;background-color:black; color: white; padding: 3mm;">Expérimenté</div>':'<div style="padding: 3mm;position:absolute;bottom:0;width:100%;background-color:white; color: black">Débutant</div>'}
    </div>`;
}